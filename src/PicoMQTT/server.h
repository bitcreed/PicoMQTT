...
public:
    ...
    const ServerSocketInterface & operator=(const ServerSocketInterface &) =
        delete;
...
protected:
    ...
    void add(Server & server) {
        servers.push_back(std::unique_ptr<ServerSocketInterface>(
            new ServerSocketProxy<Server>(server)));
    }
...
class Server : public PicoMQTTInterface,
               public Publisher,
               public SubscribedMessageListener {
public:
    class Client : public SocketOwner<std::unique_ptr<::Client>>,
                   public Connection,
                   public Subscriber {
    public:
...
protected:
    ...
    virtual ConnectReturnCode auth(const char * client_id,
                                   const char * username,
                                   const char * password) {
        return CRC_ACCEPTED;
    }
...
public:
    ...
    virtual bool publish(const char * topic, const void * payload,
                         ...
                         bool retain = false, uint16_t message_id = 0) override;
